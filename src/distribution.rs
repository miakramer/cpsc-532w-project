...
pub(crate) fn build_distribution(dtype: DistributionType, args: &[Primitive]) -> Result<Distribution, String> {
    match dtype {
        ...
        DistributionType::Categorical => {
            ...
            else if (weights.iter().copied().sum::<f32>() - 1.0).abs() < 1e-5 {
                Err("(categorical weights): weights must sum to 1".into())
            }
            else {
                Ok(Distribution::Caregorical{weights})
            }            
        }
